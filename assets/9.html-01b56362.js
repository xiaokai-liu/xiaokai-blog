import{_ as p,r as t,o as i,c as l,a as n,b as a,e,f as o}from"./app-0c97dc23.js";const c={},r=o(`<h1 id="编译器" tabindex="-1"><a class="header-anchor" href="#编译器" aria-hidden="true">#</a> 编译器</h1><ul><li>什么是编译器；</li><li>编译器的基本思路；</li><li>⼀个简单的编译器的实现；</li></ul><h2 id="什么是编译器" tabindex="-1"><a class="header-anchor" href="#什么是编译器" aria-hidden="true">#</a> 什么是编译器</h2><h3 id="编译器的介绍" tabindex="-1"><a class="header-anchor" href="#编译器的介绍" aria-hidden="true">#</a> 编译器的介绍</h3><div class="hint-container tip"><p class="hint-container-title">提示</p><p>compiler也叫编译器，是⼀种电脑程序，它会将用某种编程语言写成的源代码，转换成另⼀种编程语言。</p></div><p>从维基百科的定义来看，编译器就是个将当前语言转为其他语言的过程，回到babel上，它所做的事就是语法糖之类的转换，⽐如ES6/ES7/JSX转为ES5或者其他指定版本，因此称之为compiler也是正确的，换言之，像我们平时开发过程中所谓的其他工具，如：</p><ul><li>Less/Saas</li><li>TypeScript/coffeeScript</li><li>Eslint</li></ul><p>都可以看到compiler的身影，也是通过这些工具，才使得目前的前端工程化能⾛入相对的深⽔区，以下会详细介绍下compiler的实现思路及具体demo</p><h2 id="编译器的基本思路" tabindex="-1"><a class="header-anchor" href="#编译器的基本思路" aria-hidden="true">#</a> 编译器的基本思路</h2><h3 id="词法分析-lexical-analysis" tabindex="-1"><a class="header-anchor" href="#词法分析-lexical-analysis" aria-hidden="true">#</a> 词法分析(Lexical Analysis)</h3><h4 id="目的" tabindex="-1"><a class="header-anchor" href="#目的" aria-hidden="true">#</a> 目的</h4><p>将文本分割成⼀个个的“token”，例如：init、main、init、x、;、x、=、3、;、}等等。同时它可以去掉一些注释、空格、回车等等无效字符；</p><h4 id="生成方式" tabindex="-1"><a class="header-anchor" href="#生成方式" aria-hidden="true">#</a> 生成方式</h4><p>词法分析⽣成token的办法有2种：</p><ol><li>使用正则进⾏词法分析</li></ol><p>需要写⼤量的正则表达式，正则之间还有冲突需要处理，不容易维护，性能不高，所以正则只适合⼀些简单的模板语法，真正复杂的语言并不合适。并且有的语言并不⼀定自带正则引擎。</p><ol start="2"><li>使用自动机进⾏词法分析</li></ol><p>自动机可以很好的⽣成token；<br> 有穷状态自动机（finite state machine）：在有限个输⼊的情况下，在这些状态中转移并期望最终达到终⽌状态。<br> 有穷状态自动机根据确定性可以分为：<br> “确定有穷状态自动机”（DFA - Deterministic finite automaton）<br> 在输⼊⼀个状态时，只得到⼀个固定的状态。DFA 可以认为是⼀种特殊的 NFA；<br> “⾮确定有穷自动机”（NFA - Non-deterministic finite automaton）<br> 当输⼊⼀个字符或者条件得到⼀个状态机的集合。JavaScript 正则采用的是 NFA 引擎，具体看后⽂；</p><h3 id="语法分析-syntactic-analysis" tabindex="-1"><a class="header-anchor" href="#语法分析-syntactic-analysis" aria-hidden="true">#</a> 语法分析(Syntactic Analysis)</h3><p>我们⽇常所说的编译原理就是将⼀种语言转换为另⼀种语言。编译原理被称为形式语言，它是⼀类无需知道太多语言背景、无歧义的语言。而自然语言通常难以处理，主要是因为难以识别语言中哪些是名词哪些是动词哪些是形容词。例如：“进⼝汽⻋”这句话，“进⼝”到底是动词还是形容词？所以我们要解析⼀⻔语言，前提是这⻔语言有严格的语法规定的语言，而定义语言的语法规格称为 <strong>⽂法</strong>。</p><p>语法分析的⽬的就是通过词法分析器拿到的token流 + 结合⽂法规则，通过⼀定算法得到⼀颗抽象语法树（AST）。抽象语法树是⾮常重要的概念，尤其在前端领域应用很⼴。典型应用如babel插件，它的原理就是：<code>es6代码 → Babylon.parse → AST → babel-traverse → 新的AST → es5代码</code>。</p><p>从⽣成AST效率和实现难度上，前⼈总结主要有2种解析算法：⾃顶向下的分析方法和⾃底向上的分析方法。⾃底向上算法分析⽂法范围⼴，但实现难度⼤。而⾃顶向下算法实现相对简单，并且能够解析⽂法的范围也不错，所以⼀般的compiler都是采用深度优先索引的方式。</p><h3 id="代码转换-transformation" tabindex="-1"><a class="header-anchor" href="#代码转换-transformation" aria-hidden="true">#</a> 代码转换（Transformation）</h3><p>在得到AST后，我们⼀般会先将AST转为另⼀种AST，⽬的是⽣成更符合预期的AST，这⼀步称为代码转换。</p><p>代码转换的优势：主要是产⽣⼯程上的意义</p><ul><li>易移植：与机器无关，所以它作为中间语⾔可以为⽣成多种不同型号的⽬标机器码服务；</li><li>机器无关优化：对中间码进⾏机器无关优化，利于提高代码质量；</li><li>层次清晰：将AST映射成中间代码表示，再映射成⽬标代码的⼯作分层进⾏，使编译算法更加清晰</li></ul><p>对于⼀个Compiler而⾔，在转换阶段通常有两种形式：</p><ul><li>同语⾔的AST转换；</li><li>AST转换为新语⾔的AST；</li></ul><p>这里有⼀种通用的做法是，对我们之前的AST从上至下的解析（称为traversal），然后会有个映射表（称为visitor），把对应的类型做相应的转换。</p><h3 id="代码生成-code-generation" tabindex="-1"><a class="header-anchor" href="#代码生成-code-generation" aria-hidden="true">#</a> 代码生成(Code Generation)</h3><p>在实际的代码处理过程中，可能会递归的分析（recursive）我们最终⽣成的AST，然后对于每种type都有个对应的函数处理，当然，这可能是最简单的做法。总之，我们的⽬标代码会在这⼀步输出，对于我们的⽬标语⾔，它就是HTML了。</p><h3 id="完整链路-compiler" tabindex="-1"><a class="header-anchor" href="#完整链路-compiler" aria-hidden="true">#</a> 完整链路(Compiler)</h3><p>至此，我们就完成了⼀个完整的compiler的所有过程：</p><div class="language-javascript line-numbers-mode" data-ext="js"><pre class="language-javascript"><code><span class="token parameter">input</span> <span class="token operator">=&gt;</span> <span class="token parameter">tokenizer</span> <span class="token operator">=&gt;</span> tokens<span class="token punctuation">;</span> <span class="token comment">// 词法分析</span>
<span class="token parameter">tokens</span> <span class="token operator">=&gt;</span> <span class="token parameter">parser</span> <span class="token operator">=&gt;</span> ast<span class="token punctuation">;</span> <span class="token comment">// 语法分析，⽣成AST</span>
...
<span class="token parameter">newAst</span> <span class="token operator">=&gt;</span> <span class="token parameter">generator</span> <span class="token operator">=&gt;</span> output<span class="token punctuation">;</span> <span class="token comment">// ⽣成⽬标代码</span>
 ...
 *
 * 我们将把⼀些类似语义化代码的函数调用编译成⼀些类似 C 的函数
 * 函数调用。
 *
 ...
 *
 * 如果我们直接操作这个 AST，而不是创建⼀个单独的 AST，
 * 我们可能会在这里引⼊各种抽象。但只是参观
 ...
 *就是这样！这就是编译器的所有不同部分。
 *
 * 现在这并不是说每个编译器看起来都和我在这里描述的完全⼀样。
 * 编译器有许多不同的用途，它们可能需要更多的步骤
 * 我有详细的。
 *
 * 但是现在您应该对⼤多数编译器的外观有⼀个⼤致的高级概念
 * 喜欢。
 *

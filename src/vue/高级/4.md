...
[vue3源码官方地址](https://github.com/vuejs/core/tree/main/packages)
...
- src
    ...
    compile.ts // compile统⼀执行逻辑，有⼀个 baseCompile ，用来编译模板文件的
    index.ts // 入⼝文件
...
function traverseNode(node: any, context) {
  ...
  // i-- 这个很巧妙
  // 使用 while 是要⽐ for 快 (可以使用 https://jsbench.me/ 来测试⼀下)
  while (i--) {
    exitFns[i]();
  }
}
...
export function isReactive(value) {
  // 如果 value 是 proxy 的话
  // 会触发 get 操作，⽽在 createGetter 里面会判断
  // 如果 value 是普通对象的话
...
export function toRaw(value) {
  ...
  // TODO 这里和源码里面实现的不⼀样，不确定后面会不会有问题
...
// TODO reactive 里面如果有 ref 类型的 key 的话， 那么也是不需要调用 ref.value
的
...
export function provide(key, value) {
    ...
    if (currentInstance) {
        ...
        // 所以，如果说这里发现 provides 和 parentProvides 相等的话，那么就说明是第⼀次做 provide(对于当前组件来讲)
        // 我们就可以把 parent.provides 作为 currentInstance.provides 的原型重新赋值
        // ⾄于为什么不在 createComponent 的时候做这个处理，可能的好处是在这里初始化的话，是有个懒执行的效果（优化点，只有需要的时候在初始化）
...
function doWatch(source) {
    ...
    // 这里用 scheduler 的⽬的就是在更新的时候
    // 让回调可以在 render 前执行 变成⼀个异步的行为（这里也可以通过 flush 来改变）
...
function queueFlush() {
    ...
    // 这里就会触发两次 then （微任务逻辑）
    // 但是着是没有必要的
    // 我们只需要触发⼀次即可处理完所有的 job 调用
    // 所以需要判断⼀下 如果已经触发过 nextTick 了
    // 那么后面就不需要再次触发⼀次 nextTick 逻辑了
...
:::info
Vue3靠虚拟dom，实现跨平台的能力，runtime-dom提供⼀个渲染器，这个渲染器可以渲染虚拟dom节点到指定的容器中；
:::

### 主要功能

```js
// 源码里面这些接⼝是由 runtime-dom 来实现
// 这里先简单实现
...
:::info
可以理解成runtime-dom的延伸，,因为runtime-test对外提供的确实是dom环境的测试，⽅便用于runtime-core的测试；
:::
